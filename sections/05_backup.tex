...
\begin{frame}[t]{Position-bound diagnostic}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      \pill{Neural position range}
      \vspace{0.12cm}
      \begin{beamercolorbox}[rounded=true,sep=0.18cm]{block body}
        \centering
        {\color{Teal}\fontsize{18}{20}\selectfont\bfseries \([-5,5]\)\par}
        {\small network output range}\\[0.12cm]
        {\color{DeepNavy}\fontsize{16}{18}\selectfont\bfseries \(\max|\eta^{NN}|\approx1.33\)\par}
        {\small observed option position}
      \end{beamercolorbox}
    \end{column}
    \begin{column}{0.49\textwidth}
      \pill[Gold]{COS benchmark clip sensitivity}
      \vspace{0.12cm}
      \begin{table}
        \centering
  ...
  \vspace{0.20cm}
  \begin{beamercolorbox}[rounded=true,sep=0.14cm]{block body}
    \small The learned option positions remained well inside their imposed range; widening the analytic benchmark clip changed its RMSE only slightly.
  \end{beamercolorbox}
  \sourcefoot{Submitted report, Sec.\ 4.1.4 and Tables A.10 and A.12.}
\end{frame}
...
\begin{frame}[t]{Why the hedge option differs from the target}
  ...
  \vspace{0.20cm}
  \begin{tightitemize}
    \item \(K_h=1\) is initially at the money and variance-sensitive.
    \item \(T_h>T\) keeps the hedge instrument alive throughout the target horizon.
    \item Trading an identical target option would trivialise the hedging problem.
