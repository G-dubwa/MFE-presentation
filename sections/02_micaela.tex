...
\begin{frame}[t]{Parameter Robustness under Retraining}
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.4\textwidth}
      ...
      {\small\color{DeepNavy} above optimal for the degree-2 polynomial (vs.\ our 3--8\%)\par}
...
\end{frame}
 ...
 \begin{frame}[t]{Conditioning fixes the generalisation failure}
  \vspace{-0.05cm}
  \begin{center}
    $\delta_n = f_\theta\!\left(\log\dfrac{S_{t_n}}{K},\ \dfrac{\tau_n}{T},\ \mathbf{K},\ \boldsymbol{\sigma}\right)$
    \\[0.14cm]
    {\scriptsize\color{Slate}
     Trained on $K\sim U(0.7,1.1),\ \sigma\sim U(0.2,0.6)$ over 200k paths;
     each path priced at its own analytic BS premium (pricing separated from hedging).}
  \end{center}
  \vspace{0.14cm}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.32\textwidth}
      ...
      \vspace{0.12cm}
      {\small\color{DeepNavy}\textbf{Strike}\ } {\color{Coral}\bfseries $\leq$0.00004} {\scriptsize\color{Slate} holds}\\[0.12cm]
      {\small\color{DeepNavy}\textbf{Vol}\ } {\color{Coral}\bfseries $\sim$3$\times$} {\scriptsize\color{Slate} at $\sigma{=}0.1$}\\[0.1cm]
  ...
  \note{Card order follows the script: tested inside the range, then outside, then tail behaviour, then the efficiency win. Setup detail said aloud: strike gets its own input (volatility has no other route in; strike is technically recoverable from moneyness but explicit is easier to use), and log-moneyness now uses the true path-specific strike, no longer hardcoded. Fixed premium is justified because with r=0 the optimal premium = E[payoff] is hedge-independent, so supplying the analytic BS price isolates hedging cleanly. Interpolation nuance: the largest RELATIVE gap is at (0.7,0.2), but that is deep-OTM low-vol where absolute error is near zero. Tail risk tracking sigma is the reassuring part --- worst case is a sensible driver, and the single highest point (1.3,0.8) combines both extremes. On ``matches or beats'': at baseline the conditioned net is ~20x tighter in excess error, but that is partly the extra training data --- do not claim conditioning is intrinsically better; the unconditional win is one run + continuous coverage.}
\end{frame}
% ---------------------------------------------------------------------
