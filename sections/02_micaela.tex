...
\begin{frame}[t]{Parameter Robustness under Retraining}
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      ...
      Above DT-optimal, across all tested\\
      settings
      \par}
      \vspace{0.40cm}
      {\color{Coral}
      ...
      \vspace{0.12cm}
      {\small\color{DeepNavy}
      Worst case: benchmark shrinks\\
      faster than the network here
      \par}
    \end{column}
    %------------------------------------------------
    % Right column: graph
...
\speakerlabel{Micaela}{1:30}
\begin{frame}{Conditioning fixes the Generalisation Failure}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      {\small Inside trained range: $K\in[0.7,1.1]$, $\sigma\in[0.2,0.6]$}
      \vspace{0.14cm}

      {\color{Coral}\fontsize{19}{21}\selectfont\bfseries $\leq 0.00018$\par}
      {\scriptsize Max RMSE gap across all 9 test scenarios}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \pill{Extrapolation}
      ...
      {\small Outside trained range: $K\in\{0.6,1.3\}$, $\sigma\in\{0.1,0.8\}$}
      \vspace{0.22cm}

      {\small\color{DeepNavy}
    Strike RMSE stays within \textbf{\color{Coral}0.00004}\\[0.12cm]
        Worst vol.\ gap at $\sigma=0.1$: \textbf{\color{Coral}$\approx 3\times$}}
    \end{column}
  \end{columns}
