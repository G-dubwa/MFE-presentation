...
\begin{frame}[t,shrink=8]{Parameter Robustness under Retraining}
  \vspace{-0.3cm}
  \begin{center}
    \footnotesize
...
\begin{frame}[t]{Parameter Robustness under Retraining}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.64\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=4.08cm,keepaspectratio]
        {sections/robustness_rmse_vs_N.png}
    \end{column}
    \begin{column}{0.32\textwidth}
      \raggedright\footnotesize
      \pill{Mean neural-to-optimal RMSE ratio}
      \vspace{0.05cm}
      \metricbox{1.049}{\linewidth}
      \vspace{0.03cm}
      {\scriptsize\centering Range: \(1.009\)--\(1.099\)\par}

      \vspace{0.11cm}
      \pill{Worst relative performance}
      \vspace{0.05cm}
      \metricbox{10\%}{\linewidth}
      \vspace{0.03cm}
      {\scriptsize\centering above the discrete-time optimum\par}

      \vspace{0.11cm}
      \pill{Largest relative gap}
      \vspace{0.04cm}
      {\small\centering\(N=250\)\par}
    \end{column}
  \end{columns}
  \vspace{-0.04cm}
  \sourcefoot{Submitted report, Sec.\ 4.1.1, Table 4.1 and Figure 4.1.}
...
\begin{enumerate}
    ...
    \(K^\ast\) is built into log-moneyness, so a new strike gives the
    network the wrong input.
...
{\tiny\color{Slate}
Submitted report, Sec.\ 4.1.2 and Table 4.2.
}
...
\textbf{Inputs} &
\(\left(\log(S/K),\tau/T\right)\) &
\(\left(\log(S/K),\tau/T,K,\sigma\right)\) \\

\textbf{Training} & Fixed \(K=0.9,\ \sigma=0.4\) &
\(K\sim U(0.7,1.1)\), \(\sigma\sim U(0.2,0.6)\) \\

\textbf{Paths} & \(100\,000\) & \(200\,000\) \\

\textbf{Premium} & Learned jointly & Analytic Black--Scholes premium \\
...
\bottomrule
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \centering{\small\bfseries Interpolation}\par
      \vspace{0.03cm}
      {\scriptsize Inside the training range
      \(K\in[0.7,1.1]\), \(\sigma\in[0.2,0.6]\).\par}
      \vspace{0.03cm}
      {\footnotesize\begin{itemize}
        \setlength{\itemsep}{0.04cm}
        \item Tested across nine parameter combinations.
        \item Maximum RMSE gap: \(\boxed{\leq 0.00018}\).
        \item Performance remains close to the discrete-time optimum throughout the trained region.
      \end{itemize}}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering{\small\bfseries Extrapolation}\par
      \vspace{0.03cm}
      {\scriptsize Outside the training range
      \(K\in\{0.6,1.3\}\), \(\sigma\in\{0.1,0.8\}\).\par}
      \vspace{0.03cm}
      {\footnotesize\begin{itemize}
        \setlength{\itemsep}{0.04cm}
        \item Strike extrapolation remains within \(0.00004\) of the optimum.
        \item Volatility extrapolation is harder.
        \item At \(\sigma=0.1\), the worst RMSE is approximately \(\boxed{3\times}\) the optimum.
      \end{itemize}}
    \end{column}
  \end{columns}
  ...
  \vspace{0.04cm}
  \begin{center}
    \academicbox[0.92\textwidth]{\centering\footnotesize
      Supplying the analytic Black--Scholes premium isolates hedging error:
      any remaining error is attributable to the hedge rather than pricing.}
  \end{center}
  \vspace{-0.07cm}
  \sourcefoot{Submitted report, Sec.\ 4.1.3 and Table 4.3.}
