...
% ---------------------------------------------------------------------
% SLIDE 6 : Parameter Robustness under Retraining  (dot plot, all settings)
% ---------------------------------------------------------------------
\speakerlabel{Micaela}{1:30}
\begin{frame}[t]{Parameter Robustness under Retraining}
  \vspace{-0.05cm}
  \begin{center}\footnotesize
    ...
    from baseline $(K,\sigma,T,N)=(0.9,\,0.4,\,0.5,\,125)$.
  \end{center}
  \vspace{0.06cm}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.6\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          xmin=1.0, xmax=1.108,
          xlabel={NN / DT-optimal RMSE ratio (retrained)},
          xlabel style={font=\scriptsize,color=DeepNavy},
          xtick={1.00,1.02,1.04,1.06,1.08,1.10},
          xticklabel style={font=\scriptsize,color=Slate},
          ytick={1,2,3,4}, yticklabels={$N$,$T$,$\sigma$,$K$},
          yticklabel style={font=\small,color=DeepNavy},
          ymin=0.4, ymax=4.6,
          axis lines*=left, axis line style={color=Slate},
          xmajorgrids, grid style={color=Mist,line width=0.4pt},
          tick style={draw=none}, clip=false,
        ]
          % optimal reference
          \addplot[dashed,color=Coral,line width=0.8pt] coordinates {(1.0,0.4)(1.0,4.6)};
          % K row (y=4): 0.8,1.0,1.1,1.2
          \addplot[only marks,mark=*,mark options={fill=Teal,draw=Teal,scale=0.9}]
            coordinates {(1.082,4)(1.038,4)(1.030,4)(1.036,4)};
          % sigma row (y=3): 0.2, 0.6
          \addplot[only marks,mark=*,mark options={fill=Teal,draw=Teal,scale=0.9}]
            coordinates {(1.082,3)(1.043,3)};
          % T row (y=2): 0.25, 1.0
          \addplot[only marks,mark=*,mark options={fill=Teal,draw=Teal,scale=0.9}]
            coordinates {(1.061,2)(1.050,2)};
          % N row (y=1): 30, 60, 250
          \addplot[only marks,mark=*,mark options={fill=Teal,draw=Teal,scale=0.9}]
            coordinates {(1.009,1)(1.017,1)(1.099,1)};
          % shared baseline (1.046), one gold diamond per row
          \addplot[only marks,mark=diamond*,mark options={fill=Gold,draw=DeepNavy,scale=1.15}]
            coordinates {(1.046,4)(1.046,3)(1.046,2)(1.046,1)};
        \end{axis}
      \end{tikzpicture}
      \\[0.03cm]
      {\scriptsize\color{Slate}
       Gold $\Diamond$ = shared baseline (1.046). Every retrained setting lands in $[1.009,\,1.099]$.}
    \end{column}
    \begin{column}{0.38\textwidth}
      \centering
      {\color{Coral}\fontsize{19}{21}\selectfont\bfseries 3--8\%\par}
      {\small\color{DeepNavy} above DT-optimal across \emph{all} of $K,\sigma,T,N$\par}
      \vspace{0.3cm}
      {\color{Coral}\fontsize{19}{21}\selectfont\bfseries 1.049\par}
      {\small\color{DeepNavy} mean NN/DT ratio\par}
      \vspace{0.3cm}
      {\small\color{DeepNavy} vs.\ $\approx$\,\textbf{184\%} above optimal for the degree-2 polynomial\par}
    \end{column}
  \end{columns}
  \sourcefoot{Submitted report, Sec.\ 4.1.1, Table 4.1 and Figure 4.1.}
  \note{Each dot is one retrained setting; each row is a factor. The point is the tight cluster just above 1.0 --- reusable when retrained, across every factor, not just N. The N row has the widest spread (1.009 to 1.099): that is the benchmark improving faster as the grid refines, NOT the network degrading. Error grows with sigma and T (more accumulated variance between rebalances) and shrinks with N. Reusable when retrained --- it is not yet a single model, which is what the next slide fixes.}
\end{frame}
...
\begin{frame}{\fontsize{15}{17}\selectfont Generalization Failure of the Single-Scenario Hedger}
  ...
  \end{columns}
  \vspace{0.05cm}
  \claimbox{The inputs \emph{and} the training distribution must both change --- motivating the parameter-conditioned hedger.}
...
% ---------------------------------------------------------------------
% SLIDE 6 : Parameter Robustness under Retraining
% ---------------------------------------------------------------------
\speakerlabel{Micaela}{1:30}
\begin{frame}[t]{Parameter Robustness under Retraining}
  \vspace{-0.15cm}
  \begin{center}
    \footnotesize
    Same architecture, \textbf{retrained from scratch} at each setting (one factor at a time)
    around baseline $(K,\sigma,T,N)=(0.9,\,0.4,\,0.5,\,125)$.
    \vspace{0.05cm}

    $K\in\{0.8,\dots,1.2\}\quad \sigma\in\{0.2,0.4,0.6\}\quad
     T\in\{0.25,0.5,1.0\}\quad N\in\{30,60,125,250\}$
  \end{center}
  \vspace{0.12cm}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.56\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          xlabel={Rebalancing dates $N$},
          ylabel={NN\,/\,DT-optimal RMSE ratio},
          xlabel style={font=\scriptsize,color=DeepNavy},
          ylabel style={font=\scriptsize,color=DeepNavy},
          xtick={30,60,125,250},
          ymin=0.99, ymax=1.11,
          ...
          \addplot[color=Teal,line width=1.1pt,mark=*,
                   mark options={fill=Teal,scale=0.8}]
            coordinates {(30,1.009)(60,1.017)(125,1.046)(250,1.099)};
          \addplot[dashed,color=Coral,line width=0.8pt]
            coordinates {(20,1.0)(260,1.0)};
        \end{axis}
      \end{tikzpicture}
      \\[0.05cm]
      {\scriptsize\color{Slate}
       Absolute NN RMSE \emph{falls} $0.0156\!\to\!0.0060$ as $N$ rises;
       the ratio climbs only because DT-optimal shrinks faster.}
    \end{column}
    \begin{column}{0.4\textwidth}
      \centering
      {\color{Coral}\fontsize{20}{22}\selectfont\bfseries 3--8\%\par}
      {\small\color{DeepNavy} above the discrete-time MSE-optimal hedge, every retrained setting\par}
      \vspace{0.32cm}
      {\color{Coral}\fontsize{20}{22}\selectfont\bfseries 1.049\par}
      {\small\color{DeepNavy} mean NN/DT ratio (range 1.009--1.099)\par}
      \vspace{0.32cm}
      {\small\color{DeepNavy} vs.\ $\approx$\,\textbf{184\%} above optimal for the degree-2 polynomial baseline\par}
    \end{column}
  \end{columns}
  \vspace{0.06cm}
  \risknote{Error grows with $\sigma$ and $T$ (more accumulated variance between rebalances) and shrinks with $N$. The architecture is reusable \emph{when retrained} --- it is not yet a single model.}
  \sourcefoot{Submitted report, Sec.\ 4.1.1, Table 4.1 and Figure 4.1.}
  \note{The three RMSE curves overlap almost exactly, so I plot the NN/DT ratio instead --- that is the non-obvious result. The ratio rises with $N$ NOT because the network degrades but because the DT-optimal benchmark falls faster: the bar it is measured against gets harder to clear. Keep the polynomial comparison in the same ``\% above optimal'' units as the network (3--8\% vs $\approx$184\%); do not mix RMSE-ratios with excess-error framings.}
\end{frame}


% ---------------------------------------------------------------------
% SLIDE 7 : Generalization Failure of the Single-Scenario Hedger
% ---------------------------------------------------------------------
\speakerlabel{Micaela}{1:30}
\begin{frame}{\fontsize{15}{17}\selectfont Generalization Failure of the Single-Scenario Hedger}
  \vspace{-0.08cm}
  ...
  \claimbox{The inputs \emph{and} the training distribution must both change --- motivating the parameter-conditioned hedger.}
  \sourcefoot{Submitted report, Sec.\ 4.1.2, Table 4.2. Ratio is neural-hedge RMSE to discrete-time MSE-optimal RMSE.}
  \note{Contrast with the previous slide: there the architecture was retrained per setting and stayed within 3--8\%; here the SAME architecture is frozen and fails by up to two orders of magnitude. The three causes compound --- fixing only one would not help, which is exactly why the next slide changes inputs and training distribution together. Worst case $(0.7,0.2)$: RMSE jumps from $0.0005$ to $0.14$.}
\end{frame}
