...
\begin{frame}[t]{Conditioning fixes the generalisation failure}
  ...
  \begin{center}
    ...
    {\scriptsize\color{Slate}
     Trained on $K\sim U(0.7,1.1),\ \sigma\sim U(0.2,0.6)$, 200k paths; each path at its own analytic BS premium.}
  \end{center}
  \vspace{0.12cm}
  \begin{columns}[T,onlytextwidth]
  ...
  \note{Two regimes: inside the trained box (interpolation) and pushed outside it (extrapolation). Said aloud: strike gets its own input (volatility has no other route in; strike is recoverable from moneyness but explicit is easier to use), and log-moneyness now uses the true path-specific strike. Fixed premium justified because with r=0 the optimal premium = E[payoff] is hedge-independent, so the analytic BS price isolates hedging. Interpolation nuance: the largest RELATIVE gap is at (0.7,0.2) --- deep-OTM low-vol, absolute error near zero. On ``matches or beats'': at baseline ~20x tighter in excess error, but partly the extra training data --- the unconditional win is one run + continuous coverage.}
\end{frame}
